...
inputs = {
    secret_name = "upload-db-secret-terraform-secure-pipeline-stage"
    secret_value = jsonencode({
        host = split(":", dependency.rds.outputs.rds_endpoint)[0]
        port = split(":", dependency.rds.outputs.rds_endpoint)[1]
        username = get_env("STAGE_UPLOADS_DB_USERNAME")
        password = get_env("STAGE_UPLOADS_DB_PASSWORD")
        dbname   = get_env("STAGE_UPLOADS_DB_NAME")
  })

    tags = {
        Name              = "upload-db-secret-terraform-secure-pipeline-stage"
