...
inputs = {
  name      = "image-process-function-prod"
  use_image = true
  image_uri = get_env("PROD_IMAGE_PROCESS_IMAGE_URI", "default_image_uri")

  environment_variables = {
    RDS_SECRET_NAME = dependency.secretsmanager.outputs.secret_arn
    BUCKET_NAME = get_env("PROD_BUCKET_NAME", "default_bucket_name")
  }
  ...
  tags = {
    Name               = "image-processor-lambda-prod"
